...
const tool = async ({
    ...
    try {
        ...
        if (fs.existsSync(file)) { // warning: if under dirfile there will be broken link (pointing to something nonexisting) then this function will return false even if link DO EXIST but it's broken
            ...
            const tmp = parseInt(stats.mtimeMs, 10);

            mtimesec = parseInt(tmp / 1000, 10); // mtime sek UTC

            mtimeutc = new Date(tmp).toISOString();

            db_is_newer_then_positive = mtimesec - last_db_mtime_sec_utc;

            const condition_just_newer  = mtimesec >= last_db_mtime_sec_utc

            const condition_in_abs      = Math.abs(db_is_newer_then_positive) < time_tolerance_sec;

            if ( condition_just_newer || condition_in_abs ) {

                return {
                    mode    : 'not-expired',
                    state   : 'exist-and-not-expired-no-need-to-generate',
                    db_is_newer_then_positive,
                    full    : file,
                    mtimesec,
                    mtimeutc,
                    condition_just_newer,
                    condition_in_abs,
                    ...gen,
                }
            }
        ...
        throw th(`pdf-generator.js general error: '${String(e)}'`);
    }
};
